...
def kl_divergence(labels, outputs):
    return torch.sum(labels.exp() * (labels - outputs), dim=-1).mean()
...
def test(dataloader, model, nn_tuned_lens, tuned_lens, max_length):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(
            tqdm(
                dataloader, desc="Test:", disable=not accelerator.is_local_main_process
            )
        ):
            if batch_idx > 10:
                break
                ...
                with runner.invoke(
                    ...
                    for layer_idx, layer in enumerate(model.backbone.layers):
                        logitlens_logprobs = torch.nn.functional.log_softmax(
                            decode(layer.output, layer=layer_idx),
                            dim=-1,  # (bsz, seq_len, vocab)
                        )[:, -1, :].save()
                        ...
                        nn_tunedlens_logprobs = torch.nn.functional.log_softmax(
                            decode(layer.output, layer=layer_idx, lens=nn_tuned_lens),
                            dim=-1,
                        )[
                            :, -1, :
                        ].save()  # (bsz, 50280)
                        ...
                        nn_tuned_kls.append(
                            kl_divergence(
                                model_pred_probs, nn_tunedlens_logprobs
                            ).save()
                        )

                        tunedlens_logprobs = torch.nn.functional.log_softmax(
                            decode(layer.output, layer=layer_idx, lens=tuned_lens),
                            dim=-1,
                        )[:, -1, :].save()
            ...
            if log_losses is None:
                log_losses = {
                    f"val_logit_kl_layer_{layer_idx}": [kl]
        ...
        log_losses = {
            key: torch.tensor(value).mean() for key, value in log_losses.items()
        }
...
model = Mamba(
    "state-spaces/mamba-130m", device="cuda:0", tokenizer=tokenizer, dispatch=True
)
...
tuned_lens = TunedLens.from_model_and_pretrained(
    model, "tuned-lens/my_lenses/mamba/130m"
)
...
(model.local_model, val_dataloader) = accelerator.prepare(
    model.local_model, val_dataloader
)

logged_kls = test(val_dataloader, model, lens, tuned_lens, 25)

logit_kls = [
    logged_kls[f"val_logit_kl_layer_{i}"] for i, _ in enumerate(model.backbone.layers)
]
tuned_kls = [
    logged_kls[f"val_tuned_kl_layer_{i}"] for i, _ in enumerate(model.backbone.layers)
]
nn_tuned = [
    logged_kls[f"val_nn_tuned_kl_layer_{i}"]
